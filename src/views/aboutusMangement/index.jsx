...
function AboutUsMangement() {
  ...
  async function onSubmit(data) {
    ...
    if (totalLength > 0) {
      mutate({
        id,
        sAboutUsDescription: data.sAboutUsDescription,
        aAboutImage: aMedia
      })
    }
  }
  ...
  const { isLoading, isFetching } = useQuery(['featureById'], () => getGameFeatureById(id), {
    ...
    onSuccess: (response) => {
      ...
      setexistingImageArray(response?.aAboutImage)
    }
  })

  const { isLoading: updateLoading, mutate } = useMutation(editGameFeature, {
    onSuccess: (res) => {
      toaster('Page updated successfully')
      setPageState('view')
      setdisable(true)
          ...
          buttons={[
            {
              ...
              icon: 'edit',
              type: 'primary',
              clickEventName: 'editaboutus'
            }
          ]}
          btnEvent={() => {
            setPageState('edit')
            reset({
              sAboutUsDescription: gameFeatureData?.sAboutUsDescription
            })
          }}
        />
      )}
                    ...
                    <Col sm='12'>
                      {pageState === 'view' ? (
                        <CommonViewInput
                          ...
                          disabled
                          value={gameFeatureData?.sAboutUsDescription}
                        />
                      ) : (
                        <CommonInput
                          label='aboutus'
                          placeholder='Enter Description'
                          name='sAboutUsDescription'
                          type='textarea'
                          register={register}
                          errors={errors}
                          className={`form-control description-message${errors?.sAboutUsDescription && 'error'}`}
                          required
                          validation={{
                            required: {
                              value: true,
                              message: validationErrors.aboutusDescriptionRequired
                            },
                            maxLength: {
                              value: 1000000,
                              message: validationErrors.rangeLength(100, 100000)
                            },
                            minLength: {
                              value: 100,
                              message: validationErrors.rangeLength(100, 100000)
                      ...
                      {pageState === 'edit' && (
                        <>
                          ...
                          <Button variant='primary' type='submit' className='m-2' disabled={updateLoading || mediaUploadLoading || disable}>
                            {!mediaUploadLoading ? <FormattedMessage id='updateaboutus' /> : 'Uploading...'}
...
export default AboutUsMangement
